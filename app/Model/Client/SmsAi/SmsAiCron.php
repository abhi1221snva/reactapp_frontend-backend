...
class SmsAiCron extends Model
{
    ...
    public function addLeadTemp($id, $campaignId)
    {
        try {
            ...
            $campaign = DB::connection($this->database)->selectOne("SELECT * FROM sms_ai_campaign WHERE id = :id", array('id' => $campaignId));
    ...
    {
        $leadTemp = DB::connection($this->database)->select("SELECT campaign_id,list_id,lead_id FROM sms_ai_lead_temp WHERE campaign_id = :campaign_id", array('campaign_id' => $campaignId));
        ...
        //echo "<pre>";print_r($list);die;
                ...
                if ($record > 0)
                {
                    ...
                    foreach ( $lead as $leadKey => $value )
                    {
                        ...
                        {
                            $check_list = DB::connection($this->database)->selectOne("SELECT * FROM sms_ai_list WHERE campaign_id  = :campaign_id and id=:list_id and status=:status", array('campaign_id' => $campaignId,'list_id'=>$list->id,'status'=>'1'));
                    ...
                    DB::connection($this->database)->table('sms_ai_lead_temp')->insert($lead_data);
 ...
 */
    ...
    {
        DB::connection($this->database)->delete("DELETE FROM sms_ai_lead_temp WHERE lead_id = :lead_id AND campaign_id = :campaign_id", array('lead_id' => $leadId, 'campaign_id' => $campaignId));
    }
    ...
    {
        $leadDetail = DB::connection($this->database)->selectOne("SELECT * FROM sms_ai_list_data WHERE id = :id", array('id' => $id));
    ...
    {
        $campaignList = DB::connection($this->database)->select("SELECT * FROM sms_ai_list WHERE campaign_id = :campaign_id and status=1 ORDER BY id ASC", array('campaign_id' => $campaignId));
    ...
    public function getLead($list_id, $campaign_id, $limit = 100, $startTime = null, $endTime = null)
    {
        ...
        if (!empty($dialingColumn)) {
            $listData = DB::connection($this->database)->select("SELECT id,list_id, option_1, option_2, option_3, option_4, option_5, option_6, option_7, option_8, option_9, option_10, option_11, option_12, option_13, option_14, option_15, option_16, option_17, option_18, option_19, option_20, option_21, option_22, option_23, option_24, option_25, option_26, option_27, option_28, option_29, option_30 FROM sms_ai_list_data WHERE list_id = :list_id", array('list_id' => $list_id));
            $data = (array)$listData;
           ...
           // echo "<pre>";print_r($data);die;  // 41-50  cpu
            ...
            foreach ( $data as $key => $value ) {
                ...
                $checkTemp = DB::connection($this->database)->selectOne("SELECT lead_id FROM sms_ai_lead_temp WHERE list_id = :list_id AND campaign_id = :campaign_id AND lead_id = :lead_id", array('list_id' => $list_id, 'campaign_id' => $campaign_id, 'lead_id' => $value->id));
                $checkTemp = (array)$checkTemp;
                if (!empty($checkTemp)) continue;

                #skip is lead present in lead_report
                $checkRecord = DB::connection($this->database)->selectOne("SELECT lead_id FROM sms_ai_lead_report WHERE campaign_id = :campaign_id AND lead_id = :lead_id", array('campaign_id' => $campaign_id, 'lead_id' => $value->id));
    ...
    {
        $dialingColumn = DB::connection($this->database)->selectOne("SELECT column_name FROM sms_ai_list_header WHERE list_id = :list_id AND is_dialing = :is_dialing", array('list_id' => $list_id, 'is_dialing' => '1'));
               ...
               // echo "<pre>";print_r($response);die;
                ...
                if ($record > 0)
                {
                    ...
                    foreach ( $lead as $leadKey => $value )
                    {
                        ...
                        {
                            $check_list = DB::connection($this->database)->selectOne("SELECT * FROM sms_ai_list WHERE campaign_id  = :campaign_id and id=:id and status=:status", array('campaign_id' => $campaignId,'id'=>$list->id,'status'=>'1'));
