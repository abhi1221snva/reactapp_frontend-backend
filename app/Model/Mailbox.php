...
public function getMailbox($request)
{
    try {
        ...
        /* --------------------------------------------------------------
            EXTENSION SEARCH (OPTIONAL)
        -------------------------------------------------------------- */
        if (!empty($request->extension)) {
            $where[] = "extension = :extension";
            $search['extension'] = $request->extension;
        }

        /* --------------------------------------------------------------
            ANI SEARCH (OPTIONAL)
        -------------------------------------------------------------- */
        if (!empty($request->ani)) {
            $where[] = "ani LIKE :ani";
            $search['ani'] = "%".$request->ani."%";
        }

        /* --------------------------------------------------------------
            GLOBAL SEARCH KEYWORD (FIXED — UNIQUE PARAMS)
        -------------------------------------------------------------- */
        if (!empty($request->search)) {

            $where[] = "(
                ani LIKE :kw1 OR
                extension LIKE :kw2 OR
                vm_file_location LIKE :kw3 OR
                status LIKE :kw4 OR
                date_time LIKE :kw5
            )";

            // Bind DIFFERENT PARAMETERS
            $search['kw1'] = "%".$request->search."%";
            $search['kw2'] = "%".$request->search."%";
            $search['kw3'] = "%".$request->search."%";
            $search['kw4'] = "%".$request->search."%";
            $search['kw5'] = "%".$request->search."%";
        }
        ...
        if (!empty($request->start_date) && !empty($request->end_date)) {
            ...
            $search['start_time'] = date('Y-m-d 00:00:00', strtotime($request->start_date));
            $search['end_time']   = date('Y-m-d 23:59:59', strtotime($request->end_date));
        }

        /* --------------------------------------------------------------
            LIMIT
        -------------------------------------------------------------- */
        if (is_numeric($request->lower_limit) && is_numeric($request->upper_limit)) {
            $limitString = "LIMIT :lower_limit, :upper_limit";
            $search['lower_limit'] = intval($request->lower_limit);
            $search['upper_limit'] = intval($request->upper_limit);
        }
...
public function getMailboxn($request)
{
    try {
      ...
      if (!empty($request->search)) {
            ...
            $searchString[] = "(
                ani LIKE :keyword OR
                extension LIKE :keyword OR
                vm_file_location LIKE :keyword OR
                status LIKE :keyword OR
                date_time LIKE :keyword
            )";
        }
        /* --------------------------------------------------------------
            EXTENSION SEARCH  (OPTIONAL)
        ...
        /* --------------------------------------------------------------
            ANI SEARCH (OPTIONAL)
        -------------------------------------------------------------- */
        if ($request->has('ani') && !empty($request->ani)) {
            $search['ani'] = "%".$request->ani."%";
            $searchString[] = "ani LIKE :ani";
        }
        ...
        if ($request->start_date && $request->end_date) {
            $search['start_time'] = date('Y-m-d 00:00:00', strtotime($request->start_date));
            $search['end_time']   = date('Y-m-d 23:59:59', strtotime($request->end_date));
            $searchString[] = "date_time BETWEEN :start_time AND :end_time";
        }
        ...
        -------------------------------------------------------------- */
        if (is_numeric($request->lower_limit) && is_numeric($request->upper_limit)) {
            $search['lower_limit'] = $request->lower_limit;
            $search['upper_limit']  = $request->upper_limit;
            $limitString = "LIMIT :lower_limit, :upper_limit";
        }
        ...
        /* --------------------------------------------------------------
            BUILD QUERY
        -------------------------------------------------------------- */
        $filter = (!empty($searchString)) 
                    ? " WHERE ".implode(" AND ", $searchString)
                    : "";
