...
class RinglessVMTestController extends Controller
{
    public function index(Request $request)
    {
        ...
        $otherKey = array('Easify' => 'bc6c', 'ABF' => 'c2b9b3a0');
        ...
        $rvm_queue_list = rvmCdrLog::where('id',$rvmCdrLog_id)->get()->first();

        $rvm_data = json_decode($rvm_queue_list->json_data);
            $rvm_data->id = $rvmCdrLog_id;
       ...
       // echo "<pre>";print_r($rvm_data);die;
        ...
        if(isset($request['timezone']))
        {
            if($request['timezone'] == 0)
            {
                ...
                $phone = $rvm_data->phone;
                $cli = $rvm_data->cli;
                $voicemail_file_name = $rvm_data->voicemail_file_name;
                $apiToken = $rvm_data->apiToken;
                $user_id = $rvm_data->user_id;
                $rvm_domain_id = $rvm_data->rvm_domain_id;
                $sip_gateway_id = $rvm_data->sip_gateway_id;
                $asterisk_server_id = $rvm_data->asterisk_server_id;

                $rvm_cdr_log_id =  $rvm_data->id;


                $RvmQueueList =new RvmQueueList();
                $RvmQueueList->rvm_cdr_log_id = $rvm_data->id;
                $RvmQueueList->status = 'testing';
                $RvmQueueList->save();

                $AsteriskServer = AsteriskServer::where('rvm_status', '1')->where('id',$asterisk_server_id)->first();
                //echo "<pre>";print_r($AsteriskServer);die;
               ...
               // sleep(3);
                ...
                $originateCommand .= "Channel: Local/$phone-$cli-$voicemail_file_name-$apiToken-$user_id-$rvm_domain_id-$sip_gateway_id-$rvm_cdr_log_id@test-ringless-main\r\n";
                ...
                $rvm_cdr_log_update = RvmCdrLog::where('id', $rvm_data->id)->first();
                ...
                //dispatch((new RinglessVoicemailDropBySipName($rvm_data))->delay(Carbon::now()->addSeconds(5))->onConnection("database"));
                return array('success' => 'true',"code" => 200,'message' => 'Test Request has been accepted and is queued up.','data' => $rvm_data);
            }
