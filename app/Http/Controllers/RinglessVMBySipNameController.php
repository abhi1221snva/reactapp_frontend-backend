...
class RinglessVMBySipNameController extends Controller
{
    ...
    public function failedRvmData(Request $request)
    {
        $rvm_cdr_log_id = $request->rvm_cdr_log_id;
       
        $startTime = '07:00:00';
        $endTime = '21:00:00';
        $requestData = array();

        $rvm_cdr_log = RvmCdrLog::where('id',$rvm_cdr_log_id)->get()->first();
        $rvm_data = json_decode($rvm_cdr_log->json_data);
        $rvm_data->id = $rvm_cdr_log->id;
    ...
    {
        $this->validate($request, ['user_id' => 'required','voicemail_id' => 'required']);
        ...
        if($rvm_cdr_log)
        {
            $rvm_status['id'] = $rvm_cdr_log->id;
            $rvm_status['cli'] = $rvm_cdr_log->cli;
            $rvm_status['phone'] = $rvm_cdr_log->phone;
            $rvm_status['user_id'] = $rvm_cdr_log->user_id;
            $rvm_status['voicemail_id'] = $rvm_cdr_log->voicemail_id;
            
            $rvm_status['status'] = $rvm_cdr_log->status;
            return array('success' => 'true',"code" => 200,'message' => 'Ringless Voicemail Status.','data' => $rvm_status);
        }
        else
        {
            $rvm_status = array();
            return array('success' => 'false',"code" => 401,'message' => 'Invalid Request.','data' => $rvm_status);
        }
    }
