...
use App\Model\Client\Sip_trunk\TrunkingWallet;
use App\Model\Client\Sip_trunk\TrunkingWalletTransaction;
use App\Model\Master\Sip_trunk\TrunkingOrder;
use App\Model\Master\Sip_trunk\TrunkingPaymentTransaction;
use Exception;
...
class TrunkingRechargeController extends Controller
{
    ...
    /**
     * @OA\Post(
     *     path="/trunking/stripe/recharge",
     *     summary="Recharge client wallet using Stripe",
     *     description="Recharge wallet by using either a saved Stripe payment method or a new card.",
     *     tags={"TrunkingRecharge"},
     *     security={{"Bearer":{}}},
     *     @OA\RequestBody(
     *         required=true,
     *         @OA\JsonContent(
     ...
     *             @OA\Property(property="payment_method", type="string", example="pm_1NvxxxxxXyz", description="Use '0' if adding a new card"),
     *             
     ...
     *     @OA\Response(
     *         response=500,
     *         description="Stripe or internal server error"
     *     )
     * )
     */
    public function recharge(Request $request)
    {
        ...
        try {
            $objTrunkingMethodController = new TrunkingPaymentMethodController($request);
            ...
            $objPaymentTransactions = new TrunkingPaymentTransaction();
            ...
            if ($request->get('request_type') == 'recharge') {
                ...
                $res = TrunkingWallet::creditCharge($intTotalAmount, $request->auth->parent_id, 'USD');

                //ledger entry into client_xxx.wallet_transactions
                $objWalletTransaction = new TrunkingWalletTransaction();
