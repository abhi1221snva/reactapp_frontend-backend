...
class UnsentYesterdayRvmDropBySipNameCron extends Command
{
    protected $signature = 'app:send:unsent-yesterday-rvm-drop-by-sip-trunk  {--clientId=}';
    protected $description = 'Unsent RVM Drop Call';
    ...
    public function handle()
    {
       // die;
        ...
        $apiTokens = ['bc6c','37ea9be4-azet-6087-avlg-32bb430f3929','09eabc80-qbna-8027-wwrp-5b5f92fa7412','c2b9b3a0'];//, 'c2b9b3a0','09eabc80-qbna-8027-wwrp-5b5f92fa7412','37ea9be4-azet-6087-avlg-32bb430f3929'

        $rvm_cdr_log = RvmCdrLog::whereIn('api_token', $apiTokens)
    ...
    ->all();
       ...
       //$rvm_cdr_log = RvmCdrLog::where('api_token', 'bc6c')->where('timezone_status','1')->whereNull('status')->where('tries', '<=', 1)->where('created_at', '<',$date)->orderBy('id', 'desc')->limit(250)->get()->all();
        ...
        if($rvm_cdr_log)
        {
            foreach($rvm_cdr_log as $rvm_cdr)
            {
                ...
                $rvm_data->status_code = 'rvm_unsent_schedule_job';
                ...
                if($return["dialable"] == 1)
                {
                    ...
                    $this->info("UnsentYesterdayRVMCall rvm_unsent_schedule_job timezone is matched rvm_cdr_log table id ({$rvm_cdr->api_token},{$rvm_cdr->id})");

                }
                else
                {
                    $this->info("UnsentYesterdayRVMCall rvm_unsent_schedule_job timezone is not matched rvm_cdr_log table id ({$rvm_cdr->api_token},{$rvm_cdr->id})");
                }
            }
        ...
        {
            $this->info("UnsentYesterdayRVMCall rvm_unsent_schedule_job no data found ");
        }
