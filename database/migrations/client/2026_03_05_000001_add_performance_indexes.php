...
class AddPerformanceIndexes extends Migration
{
    public function up()
    {
        ...
        Schema::table('cdr', function (Blueprint $table) {
            ...
            $table->index(['campaign_id', 'start_time'], 'idx_cdr_campaign_start');
            $table->index('extension',      'idx_cdr_extension');
            $table->index('disposition_id', 'idx_cdr_disposition');
            $table->index('lead_id',        'idx_cdr_lead');
        });

        // ── crm_lead_data ──────────────────────────────────────────────────
        Schema::table('crm_lead_data', function (Blueprint $table) {
            // Agent/manager views always filter on assigned_to
            $table->index('assigned_to',  'idx_crm_lead_assigned_to');
            // Status filters used across dashboard, pipeline, reports
            $table->index('lead_status',  'idx_crm_lead_status');
            // Date-range filters (created_at, funding_date range queries)
            $table->index('created_at',   'idx_crm_lead_created_at');
            // Soft-delete — whereNull('deleted_at') on almost every query
            $table->index('deleted_at',   'idx_crm_lead_deleted_at');
        });
        ...
        Schema::table('lead_temp', function (Blueprint $table) {
            ...
            $table->index('list_id', 'idx_lead_temp_list');
        });
    }
